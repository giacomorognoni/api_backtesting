...
class Executor:
    ...
    def __init__(self, name,startdate,enddate):
        ...
        try:
            self.data = requests.get(f'http://api.marketstack.com/v1/tickers/{self.name}/eod', params)
        except requests.exceptions.RequestException as e:
            error = logger.error(f"Error with chosen ticker/date/request: {e}")
            return error

        self.data = self.data.json()

        self.data = pd.json_normalize(self.data['data']['eod'])
